...
import {
  ...
  TrendingUp, Layers, ChevronDown, Monitor, User2,
...
function StatsStrip() {
  ...
  useEffect(() => {
    ...
    // Use AbortController so we don't update state after unmount
...
function DashboardShowcase() {
  ...
  // Auto-advance + progress bar; resets whenever idx or tab changes
  useEffect(() => {
  ...
  // Keep active thumbnail visible — scroll within the strip, not the whole page
  useEffect(() => {
          ...
          >
            ...
            <div className="relative rounded-2xl p-px bg-gradient-to-br from-primary/35 via-primary/8 to-cyan-500/20 shadow-[0_0_100px_-20px_hsl(170_100%_42%/0.28),0_60px_120px_-30px_rgba(0,0,0,0.65)]">
              <div className="rounded-[calc(var(--radius)+7px)] overflow-hidden bg-[#060f0e]">
                ...
                <div
                  ref={thumbsRef}
                  className="flex items-center gap-2 overflow-x-auto px-4 py-3 bg-black/40 no-scrollbar"
                >
                  {shots.map((shot, i) => (
                    <button
                      key={shot.src}
